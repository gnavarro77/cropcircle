class SilburyHill extends AbstractCrop {
	...
	draw = function() {
		...
		pts.length = 0;
		p2.length = 0;
		pts.push(self.intersectionByIds(165,78)[0]);
		pts.push(self.intersectionByIds(165,38)[0]);
		group.add(lastLine(pts));
		p2.push(self.intersectionByIds(168,78)[0]);
		cercle = self.fid(78);
		group.add(self.drawArcCircle(this.getCenter(cercle), this.getRadius(cercle), p2[0], pts[0]));
		p2.push(self.intersectionByIds(168,75)[0]);
		group.add(lastLine(p2));
		p2.push(self.intersectionByIds(167,75)[0]);
		cercle = self.fid(75);
		group.add(self.drawArcCircle(this.getCenter(cercle), this.getRadius(cercle), p2[p2.length-2], p2[p2.length-1]));
		p2.push(self.intersectionByIds(167,76)[0]);
		group.add(lastLine(p2));
		p2.push(self.intersectionByIds(12,76)[0]);
		cercle = self.fid(76);
		group.add(self.drawArcCircle(this.getCenter(cercle), this.getRadius(cercle), p2[p2.length-1], p2[p2.length-2]));
		p2.push(self.intersectionByIds(12,79)[0]);
		group.add(lastLine(p2));
		p2.push(self.intersectionByIds(166,79)[0]);
		cercle = self.fid(79);
		group.add(self.drawArcCircle(this.getCenter(cercle), this.getRadius(cercle), p2[p2.length-2], p2[p2.length-1]));
		p2.push(self.intersectionByIds(166,74)[0]);
		group.add(lastLine(p2));
		p2.push(self.intersectionByIds(169,74)[0]);
		cercle = self.fid(74);
		group.add(self.drawArcCircle(this.getCenter(cercle), this.getRadius(cercle), p2[p2.length-1], p2[p2.length-2]));
		pts.push(self.intersectionByIds(170,38)[0]);
		cercle = self.fid(38);
		group.add(self.drawArcCircle(this.getCenter(cercle), this.getRadius(cercle), pts[pts.length-1], pts[pts.length-2]));
		p2.push(self.intersectionByIds(169,78)[0]);
		group.add(lastLine(p2));
		pts.push(self.intersectionByIds(170,79)[0]);
		group.add(lastLine(pts));
		
		// --- ajout trace regulateur
		pt = self.intersectionByIds(176,71)[0];
		self.drawCircle(pt,9);
		self.drawCircle(pt,13);
		self.drawCircle(pt,17);
		...
		// -- reprise
		pts.push(self.intersectionByIds(170,79)[0]);
		pts.push(self.intersectionByIds(171,79)[0]);
		group.add(lastLine(pts));
		pts.push(self.intersectionByIds(171,38)[0]);
		group.add(lastLine(pts));
		pts.push(self.intersectionByIds(254,38)[0]);
		cercle = self.fid(38);
		group.add(self.drawArcCircle(this.getCenter(cercle), this.getRadius(cercle), pts[pts.length-1], pts[pts.length-2]));
		p2.push(self.intersectionByIds(254,78)[0]);
		cercle = self.fid(78);
		group.add(self.drawArcCircle(this.getCenter(cercle), this.getRadius(cercle), p2[p2.length-1], p2[p2.length-2]));
		cercle = self.fid(254);
		group.add(self.drawArcCircle(this.getCenter(cercle), this.getRadius(cercle), p2[p2.length-1], pts[pts.length-1]));
		
		pts.length = 0;
		pts.push(self.intersectionByIds(172,79)[0]);
		pts.push(self.intersectionByIds(177,79)[0]);
		cercle = self.fid(79);
		group.add(self.drawArcCircle(this.getCenter(cercle), this.getRadius(cercle), pts[pts.length-1], pts[pts.length-2]));
		pts.push(self.intersectionByIds(177,74)[0]);
		group.add(lastLine(pts));
		pts.push(self.intersectionByIds(172,74)[0]);
		cercle = self.fid(74);
		group.add(self.drawArcCircle(this.getCenter(cercle), this.getRadius(cercle), pts[pts.length-1], pts[pts.length-2]));
		self.drawLine(pts[0],pts[pts.length-1],'trace');
		
		pts.length = 0;
		pts.push(self.intersectionByIds(164,30)[0]);
		pts.push(self.intersectionByIds(253,30)[0]);
		cercle = self.fid(30);
		group.add(self.drawArcCircle(this.getCenter(cercle), this.getRadius(cercle), pts[pts.length-1], pts[pts.length-2]));
		pts.push(self.intersectionByIds(253,37)[0]);
		cercle = self.fid(253);
		group.add(self.drawArcCircle(this.getCenter(cercle), this.getRadius(cercle), pts[pts.length-2], pts[pts.length-1]));
		pts.push(self.intersectionByIds(164,37)[0]);
		cercle = self.fid(37);
		group.add(self.drawArcCircle(this.getCenter(cercle), this.getRadius(cercle), pts[pts.length-2], pts[pts.length-1]));
		self.drawLine(pts[0],pts[pts.length-1],'trace');
		...
		pts.length = 0;
		group = self.svg.group();
		pts.push(self.intersectionByIds(122,26)[0]);
		self.drawCircle({x:pts[0].x -8,y:pts[0].y},8);
		pts.push(self.intersectionByIds(122,31)[0]);
		pts.push(self.intersectionByIds(97,26)[0]);
		cercle = self.fid(260);
		...
		cercle = self.fid(31);
		group.add(self.drawArcCircle(this.getCenter(cercle), this.getRadius(cercle), pts[2], pts[1]));
		var offset = 2 * (group.getBBox().x - self.center.x) + group.getBBox().width;
		group.clone().transform('t -'+offset+', s-1 1');
		...
		// --- fin petites plumes
		// --- petit mettoyage
		for (var i =83; i < 200; i++){
			var el = self.fid(i);
			if (el != null&& !el.hasClass('trace')){
				el.remove();
			}
		}
		
		var ref =self.drawLine({x:pt.x - 100,y:pt.y}, {x:pt.x + 100,y:pt.y});
		l = self.translateFromAToB(ref,pt, {x:pt.x,y:pt.y + 2});
		self.rotate(l,pt,90);
		for (var i =0; i < 6;i++){
			l = self.translateFromAToB(l,pt, {x:pt.x,y:pt.y + 4});
			self.rotate(l,pt,90);
		}
		l = self.translateFromAToB(ref,pt, {x:pt.x,y:pt.y - 2});
		self.rotate(l,pt,90);
...
//		pts.push(self.intersectionByIds(299,33)[0]);
//		pts.push(self.intersectionByIds(299,3)[1]);
		self.pinPoints(pts);
	}
}
